...
def app():
    ...
    with st.container():
        ...
        if 'lib_map' in st.session_state.keys():
            ...
            st.info(
                f'Below you can browse the results of two RB-TnSeq *Salmonella* screens. To analyze your own data,  please go to **⬆️ Data Upload** page. The library maps available below were generated by running `mbarq map` on raw sequencing data from [Nguyen et al 2020 study]({nguyen_2020}) and [Nguyen et al 2024 study]({nguyen_2024}). For more information about the analysis, please see [mBARq documentation]({salmonella_workflow_url}) and [mBARq paper]({mbarq_paper}).')
        ...
        with st.container():
            ...
            graph_type = st.radio(
                "Choose graph", ['Coverage Histogram', 'Individual Insertions'])
            c1, c2, c3 = st.columns(3)
            chr_col_choice = c1.selectbox(
                'Choose sequence to display', lm.lib_map[lm.chr_col].unique())
    ...
    with st.container():
        ...
        if not all([x in lm.lib_map.columns for x in ['gene_start', 'gene_end', 'gene_strand', 'percentile']]):
            ...
            gene_choice = co2.selectbox(
                'Choose gene to display', lm.lib_map[att_choice].unique())
            library_choice = co3.selectbox(
                'Choose library to display', lm.lib_map['library'].unique())
