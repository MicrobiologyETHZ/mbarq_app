...
class LibraryMap:
    ...
    def __init__(self, map_files: List = (), map_df: pd.DataFrame = pd.DataFrame()):
...
def app():
    ...
    if 'map_df' in st.session_state.keys():
        st.write(st.session_state['map_df'].head())
    else:
        st.stop()
