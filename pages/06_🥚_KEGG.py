...
def app():
    ...
    with st.container():
        ...
        if 'results_ds' in st.session_state.keys():
            ...
            st.info(
                f'Below you can browse the results of two RB-TnSeq *Salmonella* screens. To analyze your own data,  please go to **⬆️ Data Upload** page. The results available below were generated by running `mbarq analyze` on count data from the [Nguyen et al 2020 study]({nguyen_2020}) and the [Nguyen et al 2024 study]({nguyen_2024}). For more information about the analysis, please see the [mBARq documentation]({salmonella_workflow_url}) and the [mBARq paper]({mbarq_paper}).')
    ...
    if not rds.results_df.empty:
        organism_id = st.text_input('Enter the three-letter organism code', value='sey')
